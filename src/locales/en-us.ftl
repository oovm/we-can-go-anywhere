# General
app-name = We Can Go Anywhere

# Navigation
nav-home = Home
nav-about = About

# Home Page
home-title = We Can Go Anywhere
home-description = Let's embark on this wonderful journey together

# About Page
about-title = About Us
about-description = We are committed to providing you with the best experience

# Gallery
image-minecraft = Minecraft World Style
image-jojo = JoJo's Bizarre Adventure Style
image-attack-on-titan = Attack on Titan Style
image-ghibli = Studio Ghibli Style
search-placeholder = Search images...
view-on-github = View on GitHub